  ...
  static async handleGoogleCallback(code: string, redirectUri: string, companyName?: string) {
    try {
      ...
      if (existingUser) {
        ...
        const { data: authData } = await supabaseAdmin.auth.admin.getUserById(existingUser.id);
        ...
        if (companyName || isNewUser) {
          await CompanyService.createCompany({
            name: companyName || `${googleUser.name || googleUser.email.split('@')[0]}'s Workspace`,
            userId: authUser.id,
          });
        }
      }
  ...
  static async handleGitHubCallback(code: string, redirectUri: string, companyName?: string) {
    try {
      ...
      if (existingUser) {
        ...
        if (isNewUser) {
          await CompanyService.createCompany({
            name: companyName || `${githubUser.name || githubUser.login}'s Workspace`,
            userId: authUser.id,
          });
